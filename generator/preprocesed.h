...
struct ImVec2
{
    ...
    float& operator[] (size_t idx)          { ((void)( (!!(idx == 0 || idx == 1)) || (_wassert(L"idx == 0 || idx == 1", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(301)), 0)) ); return ((float*)(void*)(char*)this)[idx]; } // We very rarely use this [] operator, so the assert overhead is fine.
    float  operator[] (size_t idx) const    { ((void)( (!!(idx == 0 || idx == 1)) || (_wassert(L"idx == 0 || idx == 1", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(302)), 0)) ); return ((const float*)(const void*)(const char*)this)[idx]; }
...
namespace ImGui
{
    ...
    // Main
     ...
     ImDrawData*   GetDrawData();                              // valid after Render() and until the next call to NewFrame(). Call ImGui_ImplXXXX_RenderDrawData() function in your Renderer Backend to render.
    ...
    //  - INCORRECT: PushFont(NULL, GetFontSize() * 2.0f)       // INCORRECT! using size after global factors already applied == GLOBAL SCALING FACTORS WILL APPLY TWICE!
     void          PushFont(ImFont* font, float font_size_base_unscaled);          // Use NULL as a shortcut to keep current font. Use 0.0f to keep current size.
     void          PopFont();
     ImFont*       GetFont();                                                      // get current font
     float         GetFontSize();                                                  // get current scaled font size (= height in pixels). AFTER global scale factors applied. *IMPORTANT* DO NOT PASS THIS VALUE TO PushFont()! Use ImGui::GetStyle().FontSizeBase to get value before global scale factors.
    ...
    // Widgets: Text
     ...
     void          SeparatorText(const char* label);                               // currently: formatted text with a horizontal line
    ...
    // - You may also use one of the many IsItemXXX functions (e.g. IsItemActive, IsItemHovered, etc.) to query widget state.
     ...
     bool          TextLinkOpenURL(const char* label, const char* url = ((void *)0));     // hyperlink text button, automatically open file/url when clicked
    ...
    // - (legacy: any use of ImGuiKey will assert when key < 512 to detect passing legacy native/user indices)
     ...
     const char*   GetKeyName(ImGuiKey key);                                           // [DEBUG] returns English name of the key. Those names are provided for debugging purpose and are not meant to be saved persistently nor compared.
...
struct ImVector
{
    ...
    inline T&           operator[](int i)                   { ((void)( (!!(i >= 0 && i < Size)) || (_wassert(L"i >= 0 && i < Size", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2296)), 0)) ); return Data[i]; }
    inline const T&     operator[](int i) const             { ((void)( (!!(i >= 0 && i < Size)) || (_wassert(L"i >= 0 && i < Size", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2297)), 0)) ); return Data[i]; }

    inline T*           begin()                             { return Data; }
    inline const T*     begin() const                       { return Data; }
    inline T*           end()                               { return Data + Size; }
    inline const T*     end() const                         { return Data + Size; }
    inline T&           front()                             { ((void)( (!!(Size > 0)) || (_wassert(L"Size > 0", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2303)), 0)) ); return Data[0]; }
    inline const T&     front() const                       { ((void)( (!!(Size > 0)) || (_wassert(L"Size > 0", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2304)), 0)) ); return Data[0]; }
    inline T&           back()                              { ((void)( (!!(Size > 0)) || (_wassert(L"Size > 0", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2305)), 0)) ); return Data[Size - 1]; }
    inline const T&     back() const                        { ((void)( (!!(Size > 0)) || (_wassert(L"Size > 0", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2306)), 0)) ); return Data[Size - 1]; }
    inline void         swap(ImVector<T>& rhs)              { int rhs_size = rhs.Size; rhs.Size = Size; Size = rhs_size; int rhs_cap = rhs.Capacity; rhs.Capacity = Capacity; Capacity = rhs_cap; T* rhs_data = rhs.Data; rhs.Data = Data; Data = rhs_data; }

    inline int          _grow_capacity(int sz) const        { int new_capacity = Capacity ? (Capacity + Capacity / 2) : 8; return new_capacity > sz ? new_capacity : sz; }
    inline void         resize(int new_size)                { if (new_size > Capacity) reserve(_grow_capacity(new_size)); Size = new_size; }
    inline void         resize(int new_size, const T& v)    { if (new_size > Capacity) reserve(_grow_capacity(new_size)); if (new_size > Size) for (int n = Size; n < new_size; n++) memcpy(&Data[n], &v, sizeof(v)); Size = new_size; }
    inline void         shrink(int new_size)                { ((void)( (!!(new_size <= Size)) || (_wassert(L"new_size <= Size", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2312)), 0)) ); Size = new_size; } // Resize a vector to a smaller size, guaranteed not to cause a reallocation
    ...
    inline void         pop_back()                          { ((void)( (!!(Size > 0)) || (_wassert(L"Size > 0", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2318)), 0)) ); Size--; }
    inline void         push_front(const T& v)              { if (Size == 0) push_back(v); else insert(Data, v); }
    inline T*           erase(const T* it)                  { ((void)( (!!(it >= Data && it < Data + Size)) || (_wassert(L"it >= Data && it < Data + Size", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2320)), 0)) ); const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + 1, ((size_t)Size - (size_t)off - 1) * sizeof(T)); Size--; return Data + off; }
    inline T*           erase(const T* it, const T* it_last){ ((void)( (!!(it >= Data && it < Data + Size && it_last >= it && it_last <= Data + Size)) || (_wassert(L"it >= Data && it < Data + Size && it_last >= it && it_last <= Data + Size", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2321)), 0)) ); const ptrdiff_t count = it_last - it; const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + count, ((size_t)Size - (size_t)off - (size_t)count) * sizeof(T)); Size -= (int)count; return Data + off; }
    inline T*           erase_unsorted(const T* it)         { ((void)( (!!(it >= Data && it < Data + Size)) || (_wassert(L"it >= Data && it < Data + Size", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2322)), 0)) );  const ptrdiff_t off = it - Data; if (it < Data + Size - 1) memcpy(Data + off, Data + Size - 1, sizeof(T)); Size--; return Data + off; }
    inline T*           insert(const T* it, const T& v)     { ((void)( (!!(it >= Data && it <= Data + Size)) || (_wassert(L"it >= Data && it <= Data + Size", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2323)), 0)) ); const ptrdiff_t off = it - Data; if (Size == Capacity) reserve(_grow_capacity(Size + 1)); if (off < (int)Size) memmove(Data + off + 1, Data + off, ((size_t)Size - (size_t)off) * sizeof(T)); memcpy(&Data[off], &v, sizeof(v)); Size++; return Data + off; }
    ...
    inline int          index_from_ptr(const T* it) const   { ((void)( (!!(it >= Data && it < Data + Size)) || (_wassert(L"it >= Data && it < Data + Size", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2330)), 0)) ); const ptrdiff_t off = it - Data; return (int)off; }
};
...
struct ImGuiStyle
{
    ...
    // Functions
       ImGuiStyle();
       void ScaleAllSizes(float scale_factor); // Scale all spacing/padding/thickness values. Do not scale fonts.
...
struct ImGuiTextBuffer
{
    ...
    ImGuiTextBuffer()   { }
    inline char         operator[](int i) const { ((void)( (!!(Buf.Data != ((void *)0))) || (_wassert(L"Buf.Data != ((void *)0)", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(2860)), 0)) ); return Buf.Data[i]; }
...
struct ImTextureData
{
    ...
    ~ImTextureData()    { DestroyPixels(); }
     ...
     void      DestroyPixels();
    void*               GetPixels()                 { ((void)( (!!(Pixels != ((void *)0))) || (_wassert(L"Pixels != ((void *)0)", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(3616)), 0)) ); return Pixels; }
    void*               GetPixelsAt(int x, int y)   { ((void)( (!!(Pixels != ((void *)0))) || (_wassert(L"Pixels != ((void *)0)", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(3617)), 0)) ); return Pixels + (x + y * Width) * BytesPerPixel; }
...
struct ImFontAtlas
{
     ...
     ImFont*           AddFontFromMemoryTTF(void* font_data, int font_data_size, float size_pixels = 0.0f, const ImFontConfig* font_cfg = ((void *)0), const ImWchar* glyph_ranges = ((void *)0)); // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted after destruction of the atlas. Set font_cfg->FontDataOwnedByAtlas=false to keep ownership of your data and it won't be freed.
     ImFont*           AddFontFromMemoryCompressedTTF(const void* compressed_font_data, int compressed_font_data_size, float size_pixels = 0.0f, const ImFontConfig* font_cfg = ((void *)0), const ImWchar* glyph_ranges = ((void *)0)); // 'compressed_font_data' still owned by caller. Compress with binary_to_compressed_c.cpp.
     ImFont*           AddFontFromMemoryCompressedBase85TTF(const char* compressed_font_data_base85, float size_pixels = 0.0f, const ImFontConfig* font_cfg = ((void *)0), const ImWchar* glyph_ranges = ((void *)0));              // 'compressed_font_data_base85' still owned by caller. Compress with binary_to_compressed_c.cpp with -base85 parameter.
     void              RemoveFont(ImFont* font);

     void              Clear();                    // Clear everything (input fonts, output glyphs/textures)
    ...
    // As we are transitioning toward a new font system, we expect to obsolete those soon:
     ...
     void              ClearFonts();               // [OBSOLETE] Clear input+output font data (same as ClearInputData() + glyphs storage, UV coordinates).
    ...
    //   - ImFontAtlasCustomRect    --> Renamed to ImFontAtlasRect
     ImFontAtlasRectId AddCustomRect(int width, int height, ImFontAtlasRect* out_r = ((void *)0));// Register a rectangle. Return -1 (ImFontAtlasRectId_Invalid) on error.
...
struct ImFont
{
    ...
    ImU8                        Used8kPagesMap[(0x10FFFF+1)/8192/8]; // 1 bytes if ImWchar=ImWchar16, 16 bytes if ImWchar==ImWchar32. Store 1-bit for each block of 4K codepoints that has one active glyph. This is mainly used to facilitate iterations across all used codepoints.
...
{
    ((void)( (!!(!(_TexData != ((void *)0) && _TexID != ((ImTextureID)0)))) || (_wassert(L"!(_TexData != ((void *)0) && _TexID != ((ImTextureID)0))", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(3997)), 0)) );
...
struct ImGuiViewport
{
    ...
    ~ImGuiViewport()    { ((void)( (!!(PlatformUserData == ((void *)0) && RendererUserData == ((void *)0))) || (_wassert(L"PlatformUserData == ((void *)0) && RendererUserData == ((void *)0)", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui.h", (unsigned)(4074)), 0)) ); }
...
// Helpers: String
 ...
 void*         ImMemdup(const void* src, size_t size);                             // Duplicate a chunk of memory.
...
// Helpers: UTF-8 <> wchar conversions
 int           ImTextCharToUtf8(char out_buf[5], unsigned int c);                                                      // return output UTF-8 bytes count
...
enum ImDrawTextFlags_
{
 ...
 const char*   ImTextCalcWordWrapNextLineStart(const char* text, const char* text_end, ImDrawTextFlags flags = 0); // trim trailing space and find beginning of next line
...
struct ImBitArray
{
    ...
    bool            TestBit(int n) const        { n += OFFSET; ((void)( (!!(n >= 0 && n < BITCOUNT)) || (_wassert(L"n >= 0 && n < BITCOUNT", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(656)), 0)) ); return ((Storage[(n) >> 5] & ((ImU32)1 << ((n) & 31))) != 0); }
    void            SetBit(int n)               { n += OFFSET; ((void)( (!!(n >= 0 && n < BITCOUNT)) || (_wassert(L"n >= 0 && n < BITCOUNT", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(657)), 0)) ); ImBitArraySetBit(Storage, n); }
    void            ClearBit(int n)             { n += OFFSET; ((void)( (!!(n >= 0 && n < BITCOUNT)) || (_wassert(L"n >= 0 && n < BITCOUNT", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(658)), 0)) ); ImBitArrayClearBit(Storage, n); }
    void            SetBitRange(int n, int n2)  { n += OFFSET; n2 += OFFSET; ((void)( (!!(n >= 0 && n < BITCOUNT && n2 > n && n2 <= BITCOUNT)) || (_wassert(L"n >= 0 && n < BITCOUNT && n2 > n && n2 <= BITCOUNT", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(659)), 0)) ); ImBitArraySetBitRange(Storage, n, n2); } // Works on range [n..n2)
    bool            operator[](int n) const     { n += OFFSET; ((void)( (!!(n >= 0 && n < BITCOUNT)) || (_wassert(L"n >= 0 && n < BITCOUNT", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(660)), 0)) ); return ((Storage[(n) >> 5] & ((ImU32)1 << ((n) & 31))) != 0); }
};
...
struct  ImBitVector
{
    ...
    bool            TestBit(int n) const        { ((void)( (!!(n < (Storage.Size << 5))) || (_wassert(L"n < (Storage.Size << 5)", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(670)), 0)) ); return ((Storage.Data[(n) >> 5] & ((ImU32)1 << ((n) & 31))) != 0); }
    void            SetBit(int n)               { ((void)( (!!(n < (Storage.Size << 5))) || (_wassert(L"n < (Storage.Size << 5)", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(671)), 0)) ); ImBitArraySetBit(Storage.Data, n); }
    void            ClearBit(int n)             { ((void)( (!!(n < (Storage.Size << 5))) || (_wassert(L"n < (Storage.Size << 5)", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(672)), 0)) ); ImBitArrayClearBit(Storage.Data, n); }
};
...
struct ImSpan
{
    ...
    inline T&           operator[](int i)           { T* p = Data + i; ((void)( (!!(p >= Data && p < DataEnd)) || (_wassert(L"p >= Data && p < DataEnd", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(693)), 0)) ); return *p; }
    inline const T&     operator[](int i) const     { const T* p = Data + i; ((void)( (!!(p >= Data && p < DataEnd)) || (_wassert(L"p >= Data && p < DataEnd", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(694)), 0)) ); return *p; }
    ...
    // Utilities
    inline int  index_from_ptr(const T* it) const   { ((void)( (!!(it >= Data && it < DataEnd)) || (_wassert(L"it >= Data && it < DataEnd", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(702)), 0)) ); const ptrdiff_t off = it - Data; return (int)off; }
};
...
struct ImSpanAllocator
{
    ...
    inline void  Reserve(int n, size_t sz, int a=4) { ((void)( (!!(n == CurrIdx && n < CHUNKS)) || (_wassert(L"n == CurrIdx && n < CHUNKS", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(718)), 0)) ); CurrOff = (((CurrOff) + ((a) - 1)) & ~((a) - 1)); Offsets[n] = CurrOff; Sizes[n] = (int)sz; CurrIdx++; CurrOff += (int)sz; }
    inline int   GetArenaSizeInBytes()              { return CurrOff; }
    inline void  SetArenaBasePtr(void* base_ptr)    { BasePtr = (char*)base_ptr; }
    inline void* GetSpanPtrBegin(int n)             { ((void)( (!!(n >= 0 && n < CHUNKS && CurrIdx == CHUNKS)) || (_wassert(L"n >= 0 && n < CHUNKS && CurrIdx == CHUNKS", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(721)), 0)) ); return (void*)(BasePtr + Offsets[n]); }
    inline void* GetSpanPtrEnd(int n)               { ((void)( (!!(n >= 0 && n < CHUNKS && CurrIdx == CHUNKS)) || (_wassert(L"n >= 0 && n < CHUNKS && CurrIdx == CHUNKS", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(722)), 0)) ); return (void*)(BasePtr + Offsets[n] + Sizes[n]); }
    template<typename T>
...
struct ImStableVector
{
    ...
    inline T&           operator[](int i)           { ((void)( (!!(i >= 0 && i < Size)) || (_wassert(L"i >= 0 && i < Size", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(755)), 0)) ); return Blocks[i / BLOCK_SIZE][i % BLOCK_SIZE]; }
    inline const T&     operator[](int i) const     { ((void)( (!!(i >= 0 && i < Size)) || (_wassert(L"i >= 0 && i < Size", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(756)), 0)) ); return Blocks[i / BLOCK_SIZE][i % BLOCK_SIZE]; }
    inline T*           push_back(const T& v)       { int i = Size; ((void)( (!!(i >= 0)) || (_wassert(L"i >= 0", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(757)), 0)) ); if (Size == Capacity) reserve(Capacity + BLOCK_SIZE); void* ptr = &Blocks[i / BLOCK_SIZE][i % BLOCK_SIZE]; memcpy(ptr, &v, sizeof(v)); Size++; return (T*)ptr; }
};
...
struct ImPool
{
    ...
    ImPoolIdx   GetIndex(const T* p) const          { ((void)( (!!(p >= Buf.Data && p < Buf.Data + Buf.Size)) || (_wassert(L"p >= Buf.Data && p < Buf.Data + Buf.Size", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(776)), 0)) ); return (ImPoolIdx)(p - Buf.Data); }
...
struct ImChunkStream
{
    ...
    T*      next_chunk(T* p)            { size_t HDR_SZ = 4; ((void)( (!!(p >= begin() && p < end())) || (_wassert(L"p >= begin() && p < end()", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(808)), 0)) ); p = (T*)(void*)((char*)(void*)p + chunk_size(p)); if (p == (T*)(void*)((char*)end() + HDR_SZ)) return (T*)0; ((void)( (!!(p < end())) || (_wassert(L"p < end()", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(808)), 0)) ); return p; }
    int     chunk_size(const T* p)      { return ((const int*)p)[-1]; }
    T*      end()                       { return (T*)(void*)(Buf.Data + Buf.Size); }
    int     offset_from_ptr(const T* p) { ((void)( (!!(p >= begin() && p < end())) || (_wassert(L"p >= begin() && p < end()", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(811)), 0)) ); const ptrdiff_t off = (const char*)p - Buf.Data; return (int)off; }
    T*      ptr_from_offset(int off)    { ((void)( (!!(off >= 4 && off < Buf.Size)) || (_wassert(L"off >= 4 && off < Buf.Size", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(812)), 0)) ); return (T*)(void*)(Buf.Data + off); }
...
namespace ImGui
{
    ...
    // Fonts, drawing
     void          RegisterUserTexture(ImTextureData* tex); // Register external texture. EXPERIMENTAL: DO NOT USE YET.
...
//#endif
    ...
    inline ImGuiKey         MouseButtonToKey(ImGuiMouseButton button)                   { ((void)( (!!(button >= 0 && button < ImGuiMouseButton_COUNT)) || (_wassert(L"button >= 0 && button < ImGuiMouseButton_COUNT", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(3587)), 0)) ); return (ImGuiKey)(ImGuiKey_MouseLeft + button); }
    ...
    inline ImGuiKeyOwnerData* GetKeyOwnerData(ImGuiContext* ctx, ImGuiKey key)          { if (key & ImGuiMod_Mask_) key = ConvertSingleModFlagToKey(key); ((void)( (!!(IsNamedKey(key))) || (_wassert(L"IsNamedKey(key)", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(3613)), 0)) ); return &ctx->KeysOwnerData[key - ImGuiKey_NamedKey_BEGIN]; }
    ...
    // Debug Tools
     ...
     void          DebugNodeTexture(ImTextureData* tex, int int_id, const ImFontAtlasRect* highlight_rect = ((void *)0)); // ID used to facilitate persisting the "current" texture.
...
inline ImFontAtlasRectId ImFontAtlasRectId_Make(int index_idx, int gen_idx)     { ((void)( (!!(index_idx >= 0 && index_idx <= (0x0007FFFF) && gen_idx <= ((0x3FF00000) >> (20)))) || (_wassert(L"index_idx >= 0 && index_idx <= (0x0007FFFF) && gen_idx <= ((0x3FF00000) >> (20))", L"E:\\Repos\\i2d-imgui\\deps\\cimgui\\generator\\../imgui/imgui_internal.h", (unsigned)(4134)), 0)) ); return (ImFontAtlasRectId)(index_idx | (gen_idx << (20))); }
...
struct ImFontAtlasBuilder
{
 ...
 void              ImFontAtlasBuildClear(ImFontAtlas* atlas); // Clear output and custom rects
 ...
 void              ImFontAtlasBuildLegacyPreloadAllGlyphRanges(ImFontAtlas* atlas); // Legacy
 ...
 bool              ImFontAtlasFontInitOutput(ImFontAtlas* atlas, ImFont* font); // Using FontDestroyOutput/FontInitOutput sequence useful notably if font loader params have changed
